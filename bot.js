...
const {
  ...
  seed,
  selectTournament,
} = require("./functions/tournament.js");
const {
  assignRoles,
  capitalize,
  clearStatus,
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    if (await isNewMember(guild.id, member.user.id)) {
      ...
      return channel.send({
        content: `${member}, Welcome to the EdisonFormat.com ${EF} discord server. ${dandy}`,
      });
    } else {
      await assignRoles(guild, member);
      return channel.send({
        content: `${member}, Welcome back to the EdisonFormat.com ${EF} discord server! We missed you. ${legend}`,
      });
    }
  } catch (err) {
    console.log(err);
...
client.on("guildMemberRemove", (member) => {
  try {
    client.channels.cache
      .get(welcomeChannel)
      .send({
        content: `Oh dear. ${member.user.username} has left the server. ${sad}`,
      });
  } catch (err) {
    console.log(err);
  }
...
client.on("messageCreate", async (message) => {
  try {
    ...
    if (await isNewMember(mgid, maid))
      await createMembership(message.guild, message.member);
    ...
    if (
      ...
      const query = message.content.slice(
      ...
      if (!cardEmbed)
        return message.channel.send({
          content: `Could not find card: "${query}".`,
        });
      if (attachment) {
        return message.channel.send({
          embeds: [cardEmbed],
    ...
    if (cmd === `!fix`) {
      if (isProgrammer(message.member)) {
        ...
        if (!tournament)
          return message.channel.send({ content: `There are no tournaments.` });
        const { name } = tournament;
        await checkExpiryDate();
        await uploadDeckFolder(name);
        return message.channel.send({
          content: `Your tournament files have been uploaded! ${dandy}`,
        });
      } else {
    ...
    if (cmd === `!create`) {
      ...
      if (!args.length)
        return message.channel.send({
          content: `Please provide a name for the new tournament.`,
        });
      const tournament_type = await getTournamentType(message);
      if (!tournament_type)
        return message.channel.send({
          content: `Please select a valid tournament type.`,
        });
      ...
      try {
        ...
        try {
          ...
          console.log(err);
          return message.channel.send({
            content: `Unable to create tournament on Challonge.com.`,
          });
        }
      }
    }
    ...
    if (cmd === `!destroy`) {
      ...
      if (!args.length)
        return message.channel.send({
          content: `Please specify the name of the tournament you wish to destroy.`,
        });

      const name = args[0];
      const tournament = await Tournament.findOne({
        where: { name: { [Op.iLike]: name }, guildId: mgid },
      });
      if (!tournament)
        return message.channel.send({
          content: `Could not find tournament: "${name}".`,
        });
      if (tournament.state === "underway")
        return message.channel.send({
          content: `This tournament is underway, meaning it can only be deleted manually.`,
        });
      if (tournament.state === "complete")
        return message.channel.send({
          content: `This tournament is archived, meaning it can only be deleted manually.`,
        });
      ...
      try {
        ...
        if (status && status === 200) {
          ...
          return message.channel.send({
            content: `Yikes! You deleted ${tournament_name} ${dandy} from your Challonge account.`,
          });
        } else {
          return message.channel.send({
            content: `Unable to delete tournament from Challonge account.`,
          });
        }
      } catch (err) {
        return message.channel.send({
          content: `Error: Unable to delete tournament from Challonge account.`,
        });
      }
    }
    ...
    if (startcom.includes(cmd)) {
      ...
      const tournaments = await Tournament.findAll({
        where: { state: "pending", guildId: mgid },
        order: [["createdAt", "ASC"]],
      });
      const tournament = await selectTournament(message, tournaments, maid);
      if (!tournament)
        return message.channel.send({
          content: `There are no pending tournaments.`,
        });
      const { name, id, url } = tournament;
      const unregCount = await Entry.count({
        where: { participantId: null, tournamentId: id },
      });
      if (unregCount)
        return message.channel.send({
          content:
            "One of more players has not been signed up. Please check the Database.",
        });
      const entryCount = await Entry.count({ where: { tournamentId: id } });
      if (!entryCount)
        return message.channel.send({ content: `Error: missing entrants.` });
      const { sheet1Data, sheet2Data } = await createSheetData(tournament);
      const success = await seed(message, tournament);
      if (!success)
        return message.channel.send({
          content: `Error seeding tournament. Please try again or start it manually.`,
        });
      ...
      if (status === 200) {
        ...
        const spreadsheetId = await makeSheet(
          sheets,
          `${name} Deck Lists`,
          sheet1Data
        );
        ...
        await uploadDeckFolder(name);
        return message.channel.send({
          content: `Let's go! Your tournament is starting now: https://challonge.com/${url} ${dandy}`,
        });
      } else {
        return message.channel.send({
          content: `Error: could not access Challonge.com.`,
        });
      }
    }
    ...
    if (cmd === `!end`) {
      ...
      if (!args.length)
        return message.channel.send({
          content: `Please specify the name of the tournament you wish to end.`,
        });
      const name = args[0];
      const tournament = await Tournament.findOne({
      ...
      if (tournament.state === "complete")
        return message.channel.send({
          content: `This tournament has already ended.`,
        });
      if (!tournament)
        return message.channel.send({
          content: `Could not find tournament: "${name}".`,
      ...
      try {
        ...
        if (status && status === 200) {
          ...
          return message.channel.send({
            content: `Congrats! The results of ${tournament_name} ${dandy} have been finalized.`,
          });
        } else {
          return message.channel.send({
            content: `Unable to finalize ${tournament.name} ${dandy} on Challonge.com.`,
          });
        }
      } catch (err) {
        console.log(err);
    ...
    if (bracketcom.includes(cmd)) {
      const tournaments = await Tournament.findAll({
        where: { state: { [Op.not]: "complete" }, guildId: mgid },
        order: [["createdAt", "ASC"]],
      });
      if (!tournaments.length)
        return message.channel.send({
          content: `There are no active tournaments.`,
        });
    ...
    if (joincom.includes(cmd)) {
      ...
      if (isMod(message.member) && mention)
        return message.channel.send({
          content: `Please type **!signup @user** to register someone else for the tournament.`,
        });
      ...
      const count = await Tournament.count({
        where: { state: "underway", guildId: mgid },
      });
      const tournament = await selectTournament(message, tournaments, maid);
      if (!tournament && count)
        return message.channel.send({
          content: `Sorry, the tournament already started.`,
        });
      if (!tournament && !count)
        return message.channel.send({
      ...
      if (!info || info.status !== "free") {
        return message.channel.send(
          `Another user is submitting their deck. Please wait a bit and try **!join** again.`
        );
      } else {
      ...
      const entry = await Entry.findOne({
        where: { playerId: maid, tournamentId: tournament.id },
      });
      ...
      if (!entry) {
        ...
        if (!participant)
          return message.channel.send({
            content: `Error: Could not access tournament: ${tournament.name}`,
          });
        ...
        if (!new_entry)
          return message.channel.send({
            content: `Error: Could not access database.`,
          });
        message.member.roles.add(tourRole);
        message.author.send({
          content: `Thanks! I have all the information we need from you. Good luck in the tournament!`,
        });
        return client.channels.cache
          .get(signupChannel)
          .send({
            content: `<@${player.id}> is now registered for ${tournament.name}!`,
          });
      } else {
        await entry.update({
          url: deckListUrl,
        ...
        message.author.send({
          content: `Thanks! I have your updated deck list for the tournament.`,
        });
        return client.channels.cache
          .get(signupChannel)
          .send({
            content: `<@${player.id}> resubmitted their deck list for ${tournament.name}!`,
          });
      }
    }
    ...
    if (signupcom.includes(cmd)) {
      if (!isMod(message.member))
        return message.channel.send({
          content:
            "You do not have permission to do that. Please type **!join** instead.",
        });
      const mentioned = message.mentions.members.first();
      if (!mentioned)
        return message.channel.send({
          content: `Could not find user in the server.`,
        });
      const player = await Player.findOne({ where: { id: mentioned.user.id } });
      if (!player)
        return message.channel.send({
          content: `${mentioned.user.username} is not in the database.`,
        });
      const tournaments = await Tournament.findAll({
        where: { state: "pending", guildId: mgid },
        order: [["createdAt", "ASC"]],
      ...
      const entry = await Entry.findOne({
        where: { playerId: player.id, tournamentId: tournament.id },
      });
      ...
      const dbName = player.duelingBook
        ? player.duelingBook
        : await askForDBName(message.member, player, (override = true));
      if (!dbName) return;
      const deckListUrl = await getDeckList(
        message.member,
        player,
      ...
      if (!entry) {
        ...
        message.author.send({
          content: `Thanks! I have all the information we need for ${player.name}!`,
        });
        return client.channels.cache
          .get(signupChannel)
          .send({
        ...
        message.author.send({
          content: `Thanks! I have ${player.name}'s updated deck list for the tournament.`,
        });
        return client.channels.cache
          .get(signupcom)
          .send({
            content: `A moderator resubmitted <@${player.id}>'s deck list for ${tournament.name}!`,
          });
      }
    }
    ...
    if (dropcom.includes(cmd)) {
      ...
      if (!tournaments.length)
        return message.channel.send({
          content: `You are not in an active tournament.`,
        });
      const tournament = await selectTournament(message, tournaments, maid);
      ...
      if (!success) {
        ...
        if (!success)
          return message.channel.send({
            content: `If you played a match, please report the result before dropping. Otherwise ask a Moderator to remove you.`,
          });
      }
    ...
    if (cmd.toLowerCase() === `!remove`) {
      ...
      if (!playerId)
        return message.channel.send({
          content: `Please specify the player you wish to remove.`,
        });
      const tournaments = [
        ...(await Entry.findAll({
      ...
      if (!tournaments.length)
        return message.channel.send({
          content: `That user is not in any tournaments.`,
        });
      const tournament = await selectTournament(message, tournaments, maid);
      const entry = await Entry.findOne({
        where: { playerId: playerId, tournamentId: tournament.id },
        include: Player,
      });
      if (!entry)
        return message.channel.send({
    ...
    if (legalcom.includes(cmd)) {
      ...
      if (!info || info.status !== "free") {
        return message.channel.send(
          `Another user is submitting their deck. Please wait a bit and try **!legal** again.`
        );
      } else {
        info.status = "occupied";
        await info.save();
      ...
      if (
        ...
        issues["forbiddenCards"].length ||
        issues["limitedCards"].length ||
        issues["semiLimitedCards"].length
      ) {
        let response = `I'm sorry, ${player.name}, your deck is not legal for Edison Format. ${dandy}`;
        if (issues["illegalCards"].length)
          response += `\n\nThe following cards are not included in this format:\n${issues[
            "illegalCards"
          ].join("\n")}`;
        if (issues["forbiddenCards"].length)
          response += `\n\nThe following cards are forbidden:\n${issues[
            "forbiddenCards"
          ].join("\n")}`;
        if (issues["limitedCards"].length)
          response += `\n\nThe following cards are limited:\n${issues[
            "limitedCards"
          ].join("\n")}`;
        if (issues["semiLimitedCards"].length)
        ...
        return message.author.send({
          content: `Congrats, your Edison Format deck is perfectly legal! ${dandy}`,
        });
      }
    }
    ...
    if (banscom.includes(cmd)) {
      ...
      for (let i = 0; i < banlist.length; i += 40)
        message.author.send({
          content: banlist
            .slice(i, i + 40)
            .join("\n")
      ...
      return message.channel.send({
        content: `I messaged you the Forbidden & Limited list for Edison Format. ${dandy}`,
      });
    }
    ...
    if (dbcom.includes(cmd)) {
      ...
      if (!player) return;
      if (member && player.duelingBook)
        return message.channel.send({
          content: `${player.name}'s DuelingBook username is: ${player.duelingBook}.`,
        });
      if (member && !player.duelingBook)
        return message.channel.send({
          content: `${player.name} does not have a DuelingBook username in our the database.`,
        });
      ...
      if (marr.length > 1) {
        ...
        await player.save();
        return message.channel.send({
          content: `Your DuelingBook username has been set to: ${player.duelingBook}.`,
        });
      } else if (player.duelingBook) {
        return message.channel.send({
          content: `Your DuelingBook username is: ${player.duelingBook}.`,
        });
      } else {
        return message.channel.send({
          content: `You do not have a DuelingBook username in our database. You can type **!db** followed by your username to save it.`,
        });
      }
    }
    ...
    if (deckcom.includes(cmd)) {
      const playerId = message.mentions.users.first()
      ...
      if (!isMod(message.member) && !isAuthor) {
        ...
        if (!player)
          return message.channel.send({
            content: `${
              isAuthor ? "You are" : "That player is"
            } not in the database.`,
          });
        const tournaments = await Tournament.findAll({
          where: { state: "pending", guildId: mgid },
        ...
        if (!entry)
          return message.channel.send({
            content: `${
              isAuthor ? "You are" : "That player is"
            } not registered for ${tournament.name}.`,
          });
        message.channel.send({ content: `Please check your DMs.` });
        return message.author.send({
          content: `${player.name}'s deck link for ${tournament.name} is: ${entry.url}.`,
        });
      }
    }
    ...
    if (rolecom.includes(cmd)) {
      if (!message.member.roles.cache.some((role) => role.id === rankedRole)) {
        ...
        const membership = await Membership.findOne({
          where: { playerId: maid, guildId: mgid },
        });
        const count = await Role.count({
          where: { membershipId: membership.id, roleId: rankedRole },
        });
        if (!count) {
          await Role.create({
        ...
        return message.channel.send({
          content: `You now have the Dueling Book role.`,
        });
      } else {
        const membership = await Membership.findOne({
          where: { playerId: maid, guildId: mgid },
        ...
        return message.channel.send({
          content: `You no longer have the Dueling Book role.`,
        });
      }
    }
    ...
    if (botcom.includes(cmd)) {
      const botEmbed = new Discord.MessageEmbed()
        ...
        .setAuthor(
          "Jazz#2704",
          "https://i.imgur.com/wz5TqmR.png",
          "https://edisonformat.com/"
        )
        .setThumbnail("https://i.imgur.com/9jhJY11.png")
        .addField(
          "Ranked Play Commands",
      ...
      return message.channel.send({
        content: "I messaged you the EdisonBot User Guide.",
      });
    }
    ...
    if (cmd === `!mod`) {
      ...
      message.author.send({ embeds: [botEmbed] });
      return message.channel.send({
        content: "I messaged you the Mod-Only Guide.",
      });
    }
    ...
    if (statscom.includes(cmd)) {
      ...
      const filtered_stats = all_stats.filter((s) =>
        memberIds.includes(s.playerId)
      );
      const index = filtered_stats.length
        ? filtered_stats.findIndex((s) => s.playerId === playerId)
        : null;
      const rank = stats
        ? `#${index + 1} out of ${filtered_stats.length}`
        : `N/A`;
    ...
    if (rankcom.includes(cmd)) {
      ...
      if (x > 100 || isNaN(x))
        return message.channel.send({
          content: "Please provide a number less than or equal to 100.",
        });
      ...
      if (!top_stats.length)
        return message.channel.send({
          content: `I'm sorry, we don't have any Edison players.`,
        });
      const results = [];
      top_stats.length === 1
        ? (results[0] = `${dandy} --- The Best Edison Player --- ${dandy}`)
        : (results[0] = `${dandy} --- Top ${top_stats.length} Edison Players --- ${dandy}`);

      for (let i = 0; i < top_stats.length; i++)
        results[i + 1] = `${i + 1}. ${getMedal(top_stats[i].elo)} ${
          top_stats[i].player.name
        }`;
    ...
    if (losscom.includes(cmd)) {
      const opid = message.mentions.users.first()
        ? message.mentions.users.first().id
        : null;
      if (!opid)
        return message.channel.send({ content: `No player specified.` });
      if (opid === maid)
        return message.channel.send({
          content: `You cannot lose a match to yourself.`,
        });
      if (await isNewUser(opid))
        await createPlayer(message.mentions.members.first());
      const winner = message.guild.members.cache.get(opid);
      const winningPlayer = await Player.findOne({ where: { id: opid } });
      const wCount = await Stats.count({
        where: { playerId: opid, format: "edison" },
      });
      ...
      const lCount = await Stats.count({
        where: { playerId: maid, format: "edison" },
      });
      if (!lCount) await Stats.create({ playerId: maid, format: "edison" });
      const loserStats = await Stats.findOne({
        where: { playerId: maid, format: "edison" },
      });

      if (winner && winner.user.bot)
        return message.channel.send({
          content: `Sorry, Bots do not play Edison Format... *yet*.`,
        });
      if (opid.length < 17 || opid.length > 18)
        return message.channel.send({
          content: `To report a loss, type **!loss @opponent**.`,
        });
      if (!losingPlayer || !loserStats)
        return message.channel.send({
          content: `You are not in the database.`,
        });
      if (!winningPlayer || !winnerStats)
        return message.channel.send({
          content: `That user is not in the database.`,
        });
      ...
      const activeTournament = await Tournament.count({
        where: { state: "underway", guildId: mgid },
      });
      ...
      if (loserHasTourRole && winnerHasTourRole && activeTournament) {
        const loserEntries = await Entry.findAll({
          where: { playerId: losingPlayer.id },
          include: Tournament,
        });
        const winnerEntries = await Entry.findAll({
          where: { playerId: winningPlayer.id },
          include: Tournament,
        });
          ...
          if (common_tournament_ids.length) {
            for (let i = 0; i < common_tournament_ids.length; i++) {
              ...
              tournament = await Tournament.findOne({
                where: { id: id, guildId: mgid },
              });
              if (!tournament) continue;
              losingEntry = await Entry.findOne({
                where: {
                  playerId: losingPlayer.id,
                  tournamentId: tournament.id,
                },
              });
              winningEntry = await Entry.findOne({
                where: {
              ...
              for (let i = 0; i < matches.length; i++) {
                ...
                if (
                  checkPairing(
                    match,
                    losingEntry.participantId,
                    winningEntry.participantId
                  )
                ) {
          ...
          if (tournaments.length) {
            ...
            if (tournament) {
              ...
              if (tournament.state === "pending")
                return message.channel.send({
                  content: `Sorry, ${tournament.name} has not started yet.`,
                });
              if (tournament.state !== "underway")
                return message.channel.send({
                  content: `Sorry, ${tournament.name} is not underway.`,
                });
              const success = await processMatchResult(
                message,
                winningPlayer,
                losingPlayer,
                tournament
      ...
      const origEloWinner = winnerStats.elo || 500.0;
      const origEloLoser = loserStats.elo || 500.0;
      const delta =
        20 *
        (1 -
          (1 - 1 / (1 + Math.pow(10, (origEloWinner - origEloLoser) / 400))));
      ...
      return message.channel.send({
        content: `${losingPlayer.name}, your Edison Format ${
          isTournamentMatch ? "Tournament " : ""
        }loss to ${winningPlayer.name} has been recorded.`,
      });
    }
    ...
    if (manualcom.includes(cmd)) {
      ...
      const winnerId = message.mentions.users.first()
        ? message.mentions.users.first().id
        : args.length > 0
        ? args[0]
        : null;
      const loserId =
        userIds.length > 1 ? userIds[1] : args.length > 1 ? args[1] : null;
      if (!winnerId || !loserId)
        return message.channel.send({ content: `Please specify 2 players.` });
      if (winnerId === loserId)
        return message.channel.send({
          content: `Please specify 2 different players.`,
        });

      const winner = message.guild.members.cache.get(winnerId);
      const loser = message.guild.members.cache.get(loserId);
      if ((winner && winner.user.bot) || (loser && loser.user.bot))
        return message.channel.send({
          content: `Sorry, Bots do not play Edison Format... *yet*.`,
        });
      ...
      const wCount = await Stats.count({
        where: { playerId: winnerId, format: "edison" },
      });
      ...
      const lCount = await Stats.count({
        where: { playerId: loserId, format: "edison" },
      });
      ...
      if (!losingPlayer || !loserStats)
        return message.channel.send({
          content: `Sorry, <@${loserId}> is not in the database.`,
        });
      if (!winningPlayer || !winnerStats)
        return message.channel.send({
          content: `Sorry, <@${winnerId}> is not in the database.`,
        });
      ...
      return message.channel.send({
        content: `A manual Edison Format ${
          isTournamentMatch ? "Tournament " : ""
        }loss by ${losingPlayer.name} to ${
          winningPlayer.name
        } has been recorded.`,
      });
    }
    ...
    if (noshowcom.includes(cmd)) {
      ...
      const noShowId = message.mentions.users.first()
        ? message.mentions.users.first().id
        : args.length > 0
        ? args[0]
        : null;
      if (!noShowId)
        return message.channel.send({ content: "Please specify a player." });
      const noshow = message.guild.members.cache.get(noShowId);
      if (noshow && noshow.user.bot)
        return message.channel.send({
          content: `Sorry, Bots do not play Edison Format... *yet*.`,
        });
      ...
      if (!noShowPlayer)
        return message.channel.send({
          content: `Sorry, <@${noShowId}> is not in the database.`,
        });
      const tournaments = [
        ...(await Entry.findAll({
          where: { playerId: noShowId },
          include: Tournament,
        })),
      ]
        .map((e) => e.tournament)
      ...
      if (
        !tournaments.length ||
        !noshow.roles.cache.some((role) => role.id === tourRole)
      )
        return message.channel.send({
          content: `Sorry, ${noShowPlayer.name} is not any tournaments.`,
        });
      ...
      const noShowEntry = await Entry.findOne({
        where: { playerId: noShowId, tournamentId: tournament.id },
      });
      if (!noShowEntry)
        return message.channel.send({
          content: `Sorry I could not find that player's tournament entry in the database.`,
        });
      ...
      for (let i = 0; i < matchesArr.length; i++) {
        ...
        if (match.state !== "open") continue;
        winnerParticipantId = findNoShowOpponent(
          match,
          noShowEntry.participantId
        );
        if (winnerParticipantId) break;
      }

      const winningEntry = await Entry.findOne({
        where: {
      ...
      if (!winningEntry)
        return message.channel.send({
          content: `Error: could not find opponent.`,
        });
      ...
      return message.channel.send({
        content: `${noShowPlayer.name}, your Edison Format Tournament loss to <@${winningEntry.playerId}> has been recorded as a no-show.`,
      });
    }
    ...
    if (h2hcom.includes(cmd)) {
      const usersMap = message.mentions.users;
      const userIds = [...usersMap.keys()];
      const player1Id = message.mentions.users.first()
        ? message.mentions.users.first().id
        : null;
      ...
      if (!player1Id)
        return message.channel.send({
          content: "Please specify at least 1 other player.",
        });
      ...
      if (!player1 && player2Id === maid)
        return message.channel.send({
          content: `That user is not in the database.`,
        });
      if (!player1 && player2Id !== maid)
        return message.channel.send({
          content: `The first user is not in the database.`,
        });
      if (!player2 && player2Id === maid)
        return message.channel.send({
          content: `You are not in the database.`,
        });
      if (!player2 && player2Id !== maid)
        return message.channel.send({
          content: `The second user is not in the database.`,
        });

      const p1Wins = await Match.count({
        where: { winnerId: player1Id, loserId: player2Id, format: "edison" },
      });
      const p2Wins = await Match.count({
        where: { winnerId: player2Id, loserId: player1Id, format: "edison" },
      });
    ...
    if (undocom.includes(cmd)) {
      ...
      const prompt = isMod(message.member)
        ? ""
        : " Please get a Moderator to help you.";
      if (maid !== loserId && !isMod(message.member))
        return message.channel.send({
          content: `You did not participate in the last recorded match.${prompt}`,
        });

      if (!winnerStats.backupElo && maid !== loserId)
        return message.channel.send({
          content: `${winningPlayer.name} has no backup stats.${prompt}`,
        });
      if (!winnerStats.backupElo && maid === loserId)
        return message.channel.send({
          content: `Your last opponent, ${winningPlayer.name}, has no backup stats.${prompt}`,
        });
      if (!loserStats.backupElo && maid !== loserId)
        return message.channel.send({
          content: `${losingPlayer.name} has no backup stats.${prompt}`,
        });
      if (!loserStats.backupElo && maid === loserId)
        return message.channel.send({
          content: `You have no backup stats.${prompt}`,
        });
      ...
      return message.channel.send({
        content: `The last Edison Format match in which ${winningPlayer.name} defeated ${losingPlayer.name} has been erased.`,
      });
    }
    ...
    if (cmd === `!census`) {
      ...
      const roles = [...rolesMap.values()].reduce(
        (a, v) => ({ ...a, [v.id]: v.name }),
        {}
      );
      let updateCount = 0;
      ...
      for (let i = 0; i < members.length; i++) {
        ...
        const membership = await Membership.count({
        ...
        for (let j = 0; j < member._roles.length; j++) {
          ...
          const membership = await Membership.findOne({
            where: { playerId: member.user.id, guildId: mgid },
          });
          if (!membership) break;
          const count = await Role.count({
